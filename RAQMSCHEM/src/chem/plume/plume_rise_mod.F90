...
!     real,save,dimension(its:ite,jts:jte) :: ffirs
      ...
      rcp=r_d/cp
      nspecies=num_ebu
!      write(6,*)'num_ebu',num_ebu,'num_chem',num_chem
!      call flush(6)
!      write(6,*)'chem_opt',chem_opt,'burn_opt',burn_opt
!      call flush(6)
      ...
      if( scale_fire_emiss ) then
       ...
       if ( burn_opt == 'BIOMASSB' ) then
         ...
         do j=jts,jte
            do i=its,ite
!            if (chem_opt=='GOCART')then
!              if(ebu(i,kts,j,p_ebu_oc)/=0.0)then
!                 write(6,*)'ebu_oc top gocart ',i,kts,j,p_ebu_oc,ebu(i,kts,j,p_ebu_oc)
!              endif
!            endif
...
!            write(6,*)'num_ebu',num_ebu,'kts+1',kts+1,'kte',kte
!            call flush(6)
            do nv=1,num_ebu
              do k=kts+1,kte
