module raq_dep_ctrans_grell_mod
#include <options.h>


use raqmschem_const_mod, only : epsilc,epsilc2
...
!                         cup_dd_moisture_3d,cup_up_moisture,cup_dd_edt
  ...
  IMPLICIT NONE
  private
  public :: raq_grelldrvct
CONTAINS

!-------------------------------------------------------------
   SUBROUTINE raq_grelldrvct(DT,itimestep,                       &
              rho_phy,RAINCV,chem,trfall,              &
!              U,V,t_phy,moist,dz8w,p_phy,p8w,                   &
              U,V,t_phy,moist,p_phy,p8w,                   &
              pbl,XLV,CP,G,r_v,z,                     &
              num_chemtr,num_moist,num_chem,                     &
              ids,ide, jds,jde, kds,kde,                        &
              ims,ime, jms,jme, kms,kme,                        &
              its,ite, jts,jte, kts,kte                         )
  use chem_types_mod
  use raqmschem_species_mod,only : ichemfullpt,p_qv,p_bry,p_brcl
  use wetdep_fam_mod
  use raqmschem_pmgrid_mod,only : iam,iprnin,jprnin,iamprn,kprnin,tile,iprn,jprn,tileprn
#ifdef WETDEPDIAG
  use raqmschemcomm_mod, only : cpaveij
#endif
   use raqmschem_species_mod, only : nsol,p_rooh
!-------------------------------------------------------------
  IMPLICIT NONE
!-------------------------------------------------------------
   INTEGER,      INTENT(IN   ) ::                               &
              num_chem,num_moist,num_chemtr,                  &
                                  ids,ide, jds,jde, kds,kde,    & 
                                  ims,ime, jms,jme, kms,kme,    & 
                                  its,ite, jts,jte, kts,kte
...
!
!  our chem is only nl not ni
   REAL(CHEM_KIND_R4), DIMENSION( ims:ime , jms:jme , kts:kte, num_chemtr ),    & ! fv3 order now
         INTENT(INOUT) ::                                       &
                                   chem
   REAL(CHEM_KIND_R4), DIMENSION( ims:ime ,           jms:jme, num_chemtr ),    &
         INTENT(INOUT) ::                                       &
                                   trfall
...
!
     ...
     real,    dimension (its:ite,kts:kte,num_chem) ::                    &
           tracer,tracert
     real,    dimension (its:ite,kts:kte,nsol) :: wetdepsol  
...
!  TYPE(grid_config_rec_type),  INTENT(IN   )    :: config_flags
   INTEGER :: nv,i,j,k,ICLDCK,jpr,npr,nvfull
   REAL    :: tcrit,dp,dq
   real :: trmax
   INTEGER :: itf,jtf,ktf,iopt
   logical mask(its:ite)
   integer nmask
!   real,parameter :: epsilc=1.e-30 ! for gases e-16 is too big
!  all indices are full here dont have to start at 1
!   write(400+iam,*)'grell its',its,ite,'jts',jts,jte
!   write(400+iam,*)'grell ims',ims,ime,'jms',jms,jme
!   write(400+iam,*)'grell ids',ids,ide,'jds',jds,jde ! not used by grell
!   call flush(400+iam)
!  epsilc=1.e-30
!  return
!  jpr=40
...
!  jpr=60
!   write(6,*)'kind grell',kind(dp),kind(dt)
!  real*4 is default
!   write(200+iam,*)'top grell'
!   write(400+iam,*)'top grell'
!   call flush(200+iam)
!   call flush(400+iam)
   jpr=0
   npr=10
...
!    write(6,*)'in ctrans'
     trfall(:,:,:)=0.
!     write(400+iam,*)'j limits for grell jts,jtf ',jts,jtf
!     write(400+iam,*)' i lmits for grell its,itf ',its,itf
!     if(iam.eq.iamprn)then
!       write(250+iam,*)'p8w grell lbound',lbound(p8w),ubound(p8w)
!       write(250+iam,*)'its',its,itf,'jts',jts,jtf
!       do k=kts,kte+1
!         write(250+iam,*)'p8w',iprnin,jprnin,p8w(iprnin,k,jprnin)
!       end do
!       call flush(250+iam)
!     endif
!     if(iam.eq.4.and.tile.eq.1)then
!       write(6,*)'chem shape',shape(chem)
!       write(6,*)'lb',lbound(chem),'ub',ubound(chem) 
!       write(6,*)'jts',jts,jtf,'dust3 before',p_dust3,iprnin,jprnin,chem(iprnin,jprnin,1,p_dust3)
!       write(6,*)'its',its,itf,'kts',kts,kte
!       call flush(6)
!     endif
! #define DIAGTILE
#ifdef DIAGTILE
     if(iam.eq.iamprn)then
       write(300+iam,*)'epsilc',epsilc,'epsilc2',epsilc2
       call flush(300+iam)
       write(300+iam,*)'its',its,ite,'jts',jts,jtf,'kprnin',kprnin
       do nv=1,num_chemtr
         write(899,*)nv,'chem',chem(iprnin,jprnin,kprnin,nv)
         call flush(899)
         write(300+iam,*)nv,'chem',iprnin,jprnin,kprnin,chem(iprnin,jprnin,kprnin,nv)
         call flush(300+iam)
       end do
     endif
#endif
!     write(6,*)'grell run kts',kts,ktf,' kms ',kms,kme,' kds ',kds,kde
!     call flush(6)
#ifdef DIAGWETDEP
     if(iam.eq.iamprn)then
        write(300+iam,*)'lb chem in grell',lbound(chem),'ub',ubound(chem)
        call flush(300+iam)
     ...
     DO I=ITS,ITF
       ...
       if(iam.eq.iamprn)then
         if(i.eq.iprnin.and.j.eq.jprnin)then
           do k=1,20
                write(300+iam,*)'chem rooh 65 ',k,i,j,k,chem(i,j,k,65),'p_rooh',p_rooh
           end do
         endif
        endif
#endif
...
!           write(250+iam,*)'p8w',i,kts,j,p8w(i,kts,j),'its',its,ite,'jts',jts,jtf
!           call flush(250+iam)
!         endif
...
!
...
#ifdef DIAGWETDEP
          if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin.and.pret(i)>0.0)then
              write(6,*)'pret',pret(i)
               call flush(6)
          endif
#endif
#ifdef WETDEPDIAG
         cpaveij(i,j)=cpaveij(i,j)+pret(i)
#endif
     ...
     ENDDO
!     if(iam.eq.0)then
!     write(6,*)'shape tracer',shape(tracer)
!     write(6,*)'shape chem',shape(chem)
!     write(6,*)'num_chem',num_chem,'num_chemtr',num_chemtr
!     call flush(6)
!     endif
     tracert=0.0
!     write(6,*)'ajl num_chem',num_chem,'num_chemtr',num_chemtr
!     call flush(6)
!    loop over raqms variables
!     if(iam.eq.4.and.tile.eq.1.and.j.eq.jprnin)then
!       write(6,*)'dust3 in',p_dust3,iprnin,j,chem(iprnin,jprnin,1,p_dust3)
!       call flush(6)
!     endif
     do nv=1,num_chem
       if(ichemfullpt(nv)>0)then
         nvfull=ichemfullpt(nv)
!         write(6,*)'tracer',shape(tracer),' chem',shape(chem),'nfull',nvfull,'nv',nv
!         call flush(6)
!         if(iam.eq.4.and.tile.eq.1)then
!           write(6,*)'nv',nv,'nvfull',nvfull
!           call flush(6)
!         endif
         DO I=ITS,ITF
           trdep(i,nv)=0.
         ...
         DO K=kts,ktf
           DO I=ITS,ITF
             tracer(i,k,nv)=max(epsilc,chem(i,j,k,nvfull))
#ifdef DIAGTILE
     if(iam.eq.iamprn)then
        if(i.eq.iprnin.and.j.eq.jprnin.and.k.eq.1)then
          write(300+iam,*)'rooh top loop ',nv,chem(i,j,1,65)
        endif
       if(chempointsoluable(nv)==13)then
       if(i.eq.iprnin.and.j.eq.jprnin.and.k<=20)then
       write(300+iam,*)'nv',nv
         write(300+iam,*)nv,'nvfull',nvfull,'rooh in',iprnin,jprnin,k,chem(i,j,k,nvfull)
         call flush(300+iam)
       endif
       endif
     endif
           ...
           if(i.eq.iprnin.and.j.eq.jprnin.and.k.eq.kprnin)then
             write(300+iam,*)'epsilc',epsilc,'nv',nv,'nvfull',nvfull,tracer(i,k,nv)
             call flush(300+iam)
           endif
!              write(300+iam,*)'bry before ',tracer(i,k,nv),'nv',nv
            endif
!            if(i.eq.iprnin.and.j.eq.jprnin.and.k.eq.kprnin.and.nvfull.eq.p_brcl)then
!              write(300+iam,*)'brcl before ',tracer(i,k,nv),'nv',nv
!            endif
!            endif
!             if(isnan(tracer(i,k,nv)))then
!               write(6,*)'epsilc',epsilc,'chem',i,j,k,nvfull
!               call flush(6)
!             endif
#endif
           ENDDO
         ...
         write(6,*)'tracer missing ',nv
         call flush(6)
         call killit('tracer missing')
...
!
      CALL CUP_gf(j,kpbli,ktop,tracer,trdep,aaeq,t,q,ter11,zz,pret,p,tracert,    &
           hstary,dt,psur,us,vs,tcrit,num_chem,      &
           rho,xlv,r_v,cp,g,ipr,its,ite,itf,kts,kte,ktf,csum,xmb ,wetdepsol)
      DO I=ITS,ITF
      if(ipr(i).eq.1)then
...
!         if(j.eq.31 .and. (i.eq.92 .or. i.eq.93))then
       ...
       enddo
            ...
            DO I=its,itf
             ...
             enddo
             enddo
      CALL neg_check_ct(pret,ktop,dt,tracer,tracert,iopt,num_chem,   &
                        its,ite,kts,kte,itf,ktf,jpr,jpr,npr,j)


!lzhang diag trdep:
! tracert does not have precip or bc2
!           do i=its,itf
!             trdep(i,1)=pret(i)
...
!           enddo
       ...
       do nv=1,num_chem
         ...
         if(nvfull>0)then
!         if(iam.eq.4.and.tile.eq.1)then
!           write(6,*)'update grell ',nv,nvfull
!           call flush(6)
!         endif
            DO I=its,itf
#ifdef DIAGTILE
             if(iamprn>0)then
               if(i.eq.iprnin.and.j.eq.jprnin.and.nv.eq.1)then
                 write(300+iam,*)'pret',pret(i),'nv',nv
               endif
             endif
#endif
              if(pret(i).gt.0.)then
                 trfall(i,j,nvfull)=trdep(i,nv) !lzhang
                 trmax=maxval(chem(i,j,kts:ktf,nvfull)) ! ajl fix 9/6/2019
                 DO K=kts,ktop(i)
!                   if(isnan(tracert(i,k,nv)))then
!                     write(6,*)'tracert ',i,k,nv,'dt',dt,'nvfull',nvfull
!                   endif
#ifdef DIAGTILE
                  if(iamprn>0)then
                  if(k.eq.kprnin.and.i.eq.iprnin.and.j.eq.jprnin)then
                    write(300+iam,*)'chem before',i,j,k,'nvfull',nvfull,chem(i,j,k,nvfull),'tracert',tracert(i,k,nv)
!                    write(250+iam,*)'chem before',i,j,k,'nvfull',nvfull,chem(i,j,k,nvfull),'tracert',tracert(i,k,nv)
!                    call flush(250+iam)
                  endif
                  endif
#endif
!                 add check for min from gsdchem 0.8.7
!                  if(nvfull.eq.p_co)then
!                    if(i.eq.iprnin.and.j.eq.jprnin)then
!                      write(6,*)'co before ',i,j,k,chem(i,j,k,nvfull),'add ','nv',nv,tracert(i,k,nv)*dt
!                      write(6,*)'trmax',trmax
!                    endif
!                  endif
                   chem(i,j,k,nvfull)=min(trmax,chem(i,j,k,nvfull)+tracert(i,k,nv)*dt)
                   chem(i,j,k,nvfull)=max(epsilc,chem(i,j,k,nvfull))
!                  if(nvfull.eq.p_co)then
!                    if(i.eq.iprnin.and.j.eq.jprnin)then
!                      write(6,*)'co after add ',k,chem(i,j,k,nvfull),'nvfull',nvfull
!                    endif
!                  endif
#ifdef DIAGTILE
                  if(iamprn>0)then
                  if(k.le.kprnin.and.i.eq.iprnin.and.j.eq.jprnin)then
                    write(300+iam,*)'chem after',i,j,k,'nvfull',nvfull,chem(i,j,k,nvfull)
!                    write(250+iam,*)'chem after',i,j,k,'nvfull',nvfull,chem(i,j,k,nvfull)
!                    call flush(250+iam)
                  endif
                  endif
!                   if(isnan(chem(i,j,k,nvfull)))then
!                     write(6,*)'chem nan grell',i,j,k,nvfull,chem(i,j,k,nvfull),'nv',nv
!                     call flush(6)
!                   endif
#endif
                 ENDDO
       ...
       ENDDO
!     if(iam.eq.4.and.tile.eq.1.and.j.eq.jprnin)then
!       write(6,*)'dust3 out grell',p_dust3,iprnin,j,chem(iprnin,jprnin,1,p_dust3)
!       call flush(6)
!     endif
!       call wetdepcumfam(num_chem,its,ite,jts,jte,kts,kte,dt,chem,tracert,j)
!       if(iam.eq.iamprn.and.j.eq.jprnin)then
!         write(500+iam,*)'num_chem',num_chem
!         call flush(500+iam)
!         do k=1,10
!            !write(300+iam,*)'wetdepsol 1 in',wetdepsol(iprnin,k,1)
!            write(500+iam,*)'wetdepsol 1 in',wetdepsol(iprnin,k,1)
!            call flush(500+iam)
!            call flush(300+iam)
!         end do
!       endif
!       write(6,*)'num_chem',num_chem,' shape chem',shape(chem),'num_chemtr',num_chemtr
!       call flush(6)         
!      ajl 10/17/2019 wetdepsol is a j slice so must be inside j loop
       call wetdepcumfam(num_chemtr,its,ite,jts,jte,kts,kte,dt,chem,wetdepsol,j)
 100    continue ! try 9/13/2019 ajl cant do this since j below 10/17/2019
!       if(iam.eq.iamprn.and.j.eq.jprnin)then
!         write(6,*)'co before wetdepcumfam ',j,chem(iprnin,jprnin,1,p_co),chem(iprnin,jprnin,64,p_co)
!       endif
!      add 10/17/2019
!       call wetdepcumfam(num_chemtr,its,ite,jts,jte,kts,kte,dt,chem,wetdepsol,j)
!       if(iam.eq.iamprn.and.j.eq.jprnin)then
!         write(6,*)'co after wetdepcumfam ',j,chem(iprnin,jprnin,1,p_co),chem(iprnin,jprnin,64,p_co)
!       endif
!       if(iamprn)then
!       endif
...
!100    continue
!     if(iam.eq.4.and.tile.eq.1)then
!       write(6,*)'dust3 out after wetdepdum',p_dust3,iprnin,jprnin,chem(iprnin,jprnin,1,p_dust3)
!       call flush(6)
!     endif
     return
   END SUBROUTINE raq_GRELLDRVCT
...
!
   SUBROUTINE CUP_gf(j,kpbli,ktop,chem,chem_psum,aaeq,t,q,z1,z,pre,p,outc,    &
              hstary,DTIME,PSUR,US,VS,TCRIT,numc, &
              rho,xlv,r_v,cp,g,ipr,its,ite,itf,kts,kte,ktf,csum,xmb ,wetdepsol)
   use raqmschem_species_mod, only : chemsoluable,nsol, &
      chempointsoluable
   use wetdep_alpha_mod, only : solalpha
   use raqmschem_pmgrid_mod, only : iprnin,jprnin,kprnin,iam,iamprn
   IMPLICIT NONE
   real,    dimension (its:ite,kts:kte,nsol) :: wetdepsol  

     integer                                                           &
        ,intent (in   )                   ::                           &
        numc,its,ite,itf,kts,kte,ktf,j
  !
  ! 
  !
...
!    ajl some diagnostics to check on wet dep part
!     real,    dimension (its:ite,kts:kte,numc)   :: dellactran,wetdeppart
     real,    dimension (its:ite,kts:kte,numc)   :: dellactran
     real,    dimension (its:ite,kts:kte,numc)   :: wetdeppart
     real,    dimension (its:ite,numc) :: sumdellactran,sumwetdeppart                          
...
!        hcot,dby,qc,qrcd,pwd,pw,hcd,qcd,dbyd,hc,qrc,zu,zd,clw_all
        hcot,dby,qc,qrcd,pwd,pw,hcd,dbyd,hc,qrc,zu,zd,clw_all
!     real*8, dimension (its:ite,kts:kte) :: qcd
     real zdtmp(kts:kte)
     real, dimension (its:ite,kts:kte) :: qcd
...
!      real :: alpha,trcc,rain,t1,q1,elocp,evef,el2orc,evfact,evfactl,g_rain,e_dn,c_up
      real :: trcc,rain,t1,q1,elocp,evef,el2orc,evfact,evfactl,g_rain,e_dn,c_up
...
! ! no restriction if pre incoming is non zero
      ...
      do i=its,itf
        ...
        ierr(i)=0
        cap_max(i)=cap_maxs
!        cap_max_increment(i)=25. ! ajl add from WRF
        cap_max_increment(i)=20. ! new from gsdchem 8.7
        ierrc(i)=" "
      enddo
!
...
!
      ...
      do i=its,itf
        ...
        zd(i,k)=0.
        hcot(i,k)=0.
        he(i,k)=0.0 ! new for 8.8
        cd(i,k)=1.e-9 ! 1.*entr_rate
!        cdd(i,k)=1.e-9
        cdd(i,k)=0.0 ! gsdchem 8.7
      enddo
      enddo
!
...
!
      ...
      if(iam.eq.iamprn)then
       if(i.eq.iprnin.and.j.eq.jprnin)then
          write(300+iam,*)'t1',t(iprnin,1),'qes',qes(iprnin,1),'z1',z1(iprnin)
          call flush(300+iam)
       endif
      endif
#endif
...
!        if(i.eq.iprnin.and.j.eq.jprnin)then
!          do k=kts,ktf
!            write(300+iam,*)'hes_cup',k,hes_cup(i,k)
!          end do
!!            write(250+iam,*)k,'p_cup',p_cup(i,k)
!!            call flush(6)
!!          end do
!        endif
!        endif
...
!        if(iam.eq.iamprn)then
!        if(i.eq.iprnin.and.j.eq.jprnin)then
!             write(300+iam,*)'err at 55 ',ierr(i)
!        endif
!        endif
         if(ierr(i).eq.0)then
         hc(i,k22(i))=hkb(i)
...
!
      DO 37 i=its,itf
         kzdown(i)=0
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!        write(300+iam,*)'at 66 ierr',ierr(i),i
!        call flush(300+iam)
!        endif
         if(ierr(i).eq.0)then
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!        write(300+iam,*)'at 667 ierr',ierr(i),i
!        call flush(300+iam)
!        endif
         IF(ierr(I).eq.0.)THEN
!
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!        write(300+iam,*)'at 668 ierr',ierr(i),i
!        call flush(300+iam)
!        endif
         IF(ierr(I).eq.0.)THEN
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!        write(300+iam,*)'at 6610 ierr',ierr(i),i
!        call flush(300+iam)
!        endif
            endif
         endif
...
!--- in this routine
!
!      if(iamprn>0.and.j.eq.jprnin)then
!        write(300+iam,*)'top loop j ',j,kts,ktf,'its',its,itf
!        call flush(300+iam)
!      endif
      do k=kts,ktf
      do i=its,itf
      ...
      enddo
!      if(iamprn>0.and.j.eq.jprnin)then
!        write(300+iam,*)'bottom loop j ',j,kts,ktf,'its',its,itf,'iprnin',iprnin,jprnin
!        call flush(300+iam)
!      endif
      do i=its,itf
!        if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!          write(300+iam,*)'at yyy i',i,'its',its,itf,'ierr',ierr(i)
!          call flush(300+iam)
!        endif
...
!            if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!            write(300+iam,*) 'ierr at 44',ierr(i)
!            endif
        IF(ierr(I).eq.0)then
...
!           call get_zu_zd_pdf_fim("DOWN",ierr(i),kdet(i),jmin(i),zd(i,:),kts,kte,ktf,beta,kpbli(i),csum(i))
           call get_zu_zd_pdf_fim("DOWN",ierr(i),kdet(i),jmin(i),zdtmp,kts,kte,ktf,beta,kpbli(i),csum(i))
        zd(i,:)=zdtmp
         if(ipr(i).eq.1)then
            do k=kts,ktop(i)
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!               write(300+iam,*)'zd min',jmin(i),zd(i,jmin(i))
!              call flush(300+iam)
!           endif
        if(zd(i,jmin(i)) .lt.1.e-8)then
          ...
          if(zd(i,jmin(i)) .lt.1.e-8)then
             ierr(i)=876
!             exit
             cycle ! ajl should do rest of i's
          endif
        endif
!        if(iamprn>0.and.j.eq.jprnin)then
!           write(300+iam,*)'at qqq i',i
!           call flush(300+iam)
!        endif
...
!-srf mass cons
        ...
        enddo
!            if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!                  write(300+iam,*)'at a jmin',jmin(i)
!             endif
        ...
        enddo
!            if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!              write(300+iam,*)'ajl at ff kts',kts,'ktop(i',ktop(i)
!              call flush(300+iam)
!            endif
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!             write(300+iam,*)'at ajlbb jmin',i,jmin(i)
!             call flush(300+iam)
!           endif
            do ki=jmin(i)  ,1,-1
!-srf mass cons
...
!           if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!               write(300+iam,*)'add hcd ',i,ki,hcd(i,ki)
!               call flush(300+iam)
!           endif
...
!
      call cup_dd_moisture_new(j,ierrc,zd,hcd,hes_cup,qcd,qes_cup, &
...
!             if(nmask>0)then
!               write(6,*)'mask grell',nmask
!               call flush(6)
!             endif
             if(nmask>0)then
!              have a least one i with alpha
!                do k=k22(i)+1,ktop(i)
!               write(6,*)'dep dcall solalpah',shape(alphasoluable),kind(alphasoluable)
!               call flush(6)
!               call solalpha(its,itf,kts,kte,mask,k22,ktop,j,t,qcd,alphasoluable)
               call solalpha(its,itf,kts,kte,mask,k22,ktop,j,t,alphasoluable)
             endif
             do i=its,itf
               ...
               if(ierr(i).eq.0)then
                ...
                edt(i)=edtc(i,1)
              do nv=1,numc
!              here are raqms tracers only
!              need pointer to full array
!               if(lraqmschem(nv))then
                if(chemsoluable(nv))then
                  do k=k22(i),ktop(i)
                    alpha(k)=alphasoluable(i,k,chempointsoluable(nv))
#ifdef DIAGWETDEP
...
#endif
!                     write(300+iam,*)'nv',nv,'alpha',alpha(k),chempointsoluable(nv)
!                     call flush(6)
!                    endif
!                    if(isnan(alpha(k)))then
!                        write(6,*)'nan alpha ',i,j,k,nv
!                        !call flush(6)
!                    endif
                  end do
                else
...
!                   if(isnan(chem_cup(i,k,nv)))then
!                       write(6,*)'chem cup def',i,k,nv
!                       call flush(6)
!                   endif
                enddo
...
!
                ...
                do k=k22(i)+1,ktop(i)
                   chem_up(i,k,nv)=(chem_up(i,k-1,nv)*zu(i,k-1) &
                         ...
                         (zu(i,k-1)-.5*up_massdetr(i,k-1)+up_massentr(i,k-1))
!                  if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!                      write(300+iam,*)'chem_up aa ',chem_up(i,k,nv),i,k,nv
!                  endif
             
                   chem_c(i,k,nv)=alpha(k)*chem_up(i,k,nv)
!                   if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin.and.k.eq.kprnin)then
!                     write(300+iam,*)'chem_c',i,k,nv,chem_c(i,k,nv)
!                     call flush(300+iam)
!                   endif
!                   if(isnan(chem_c(i,k,nv)))then
!                     write(6,*)'nan chem_c',i,k,nv
!                     call flush(6)
!                   endif
                   dz=z_cup(i,K)-z_cup(i,K-1)
...
!                  if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!                    write(250+iam,*)'trcc',trcc,'pw',chem_pw(i,k,nv),'chem_up',chem_up(i,k,nv)
!                    write(300+iam,*)'trcc',trcc,'pw',chem_pw(i,k,nv),'chem_up',chem_up(i,k,nv)
!                    call flush(250+iam)
!                  endif
                enddo
!                    if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!                       write(250+iam,*)'ktop',ktop(i)
!                    endif
                do k=ktop(i)+1,ktf
                 chem_up(i,k,nv)=chem_cup(i,k,nv)
                enddo
...
!       if(lraqmschem(nv))then
        do i=its,itf
!         if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!           write(300+iam,*)'pre grell ',pre(i),'ierr',ierr(i),'ktop',ktop(i),ierrc(i)
!           call flush(300+iam)
!         endif
          if(ierr(i).eq.0)then
            ...
            dellac(i,1,nv)=( edt(i)*zd(i,2)*chem_down(i,2,nv)   &
                    ...
                    -zu(i,2)*(chem_up(i,2,nv)-chem_cup(i,2,nv)) )*g/dp &
                         - g_rain + E_dn
#ifdef DIAGDELL
            dellactran(i,1,nv)=(edt(i)*zd(i,2)*chem_down(i,2,nv)   &
                     -edt(i)*zd(i,2)*chem_cup(i,2,nv) &
                   -zu(i,2 )*(chem_up (i,2,nv  )-chem_cup(i,2,nv)) &
                                          )*g/dp 
#endif
            wetdeppart(i,1,nv)=-g_rain+E_dn
#ifdef DIAGDELL
            k=2
            if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
            write(500+iam,*)'k',k,'zu up',zu(i,k)*chem_up(i,k,nv),'zu cup ',-zu(i,k)*chem_cup(i,k,nv)
            write(500+iam,*)k,'zu term',-zu(i,2 )*(chem_up (i,2,nv)-chem_cup(i,2,nv))
            write(500+iam,*)k,'zd term',edt(i)*zd(i,2)*(chem_down(i,2,nv)-chem_cup(i,2,nv))
            write(500+iam,*)'k',k,'zd dn',-zd(i,k)*chem_down(i,k,nv)*edt(i),'zd cup',zd(i,k)*chem_cup(i,k,nv)*edt(i)
            write(300+iam,*)'i',i,'nv',nv,'dellactran',dellactran(i,1,nv),wetdeppart(i,1,nv)
            write(500+iam,*)'i',i,'nv',nv,'top 2',dellactran(i,1,nv)
            call flush(400+iam)
            endif
#endif
!     real,    dimension (its:ite,kts:kte,numc)   :: dellactran,wetdeppart
!           if(isnan(dellac(i,1,nv)))then
!             write(250+iam,*)'dellac1',dellac(i,1,nv),'edt',edt(i)
!             write(250+iam,*)'chem_down',chem_down(i,2,nv),'zd',zd(i,2)
!             write(250+iam,*)'chem_cup',chem_cup(i,2,nv),'dp',dp
!             write(250+iam,*)'p_cup',p_cup(i,1),p_cup(i,2)
!             write(250+iam,*)'g',g,g_rain,'E_dn',E_dn
!             call flush(250+iam)
!           endif
...
!             call flush(250+iam)
!           endif
            if(zu(i,ktop(i)+1).ne.0.0)then
              write(6,*)'zu ktop ',i,ktop(i)+1,zu(i,ktop(i)+1)
              call flush(6)
            endif
            do k=kts+1,ktop(i)
...
!                                 zu(i,k  )* qrco(i,k  )  )*g/dp + G_rain
               ...
               dellac(i,k,nv) =-(zu(i,k+1)*(chem_up(i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    ...
                    zd(i,k  )*(chem_down(i,k,nv  )-chem_cup(i,k,nv)))*g/dp*edt(i) &
                         - g_rain + E_dn
#ifdef DIAGDELL
               dellactran(i,k,nv) =-(zu(i,k+1)*(chem_up(i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    zu(i,k  )*(chem_up (i,k,nv  )-chem_cup(i,k,nv  ) ) )*g/dp &
                    +(zd(i,k+1)*(chem_down(i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    zd(i,k  )*(chem_down(i,k,nv  )-chem_cup(i,k,nv)))*g/dp*edt(i) 
            if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
               if(k.eq.kts+1)then
                 write(500+iam,*)'k',k,'zu up',zu(i,k)*chem_up(i,k,nv),'zu cup ',-zu(i,k)*chem_cup(i,k,nv)
                 write(500+iam,*)k,'zup term',k,zu(i,k)*(chem_up(i,k,nv)-chem_cup(i,k,nv))
                 write(500+iam,*)'k',k,'zd dn',-zd(i,k)*chem_down(i,k,nv)*edt(i),'zd cup',zd(i,k)*chem_cup(i,k,nv)*edt(i)
                 write(500+iam,*)k,'zdn term',k,-zd(i,k)*(chem_down(i,k,nv)-chem_cup(i,k,nv))*edt(i)
               endif
            endif
#endif
               wetdeppart(i,k,nv)=-g_rain+E_dn
#ifdef DIAGDELL
               if(iam.eq.iamprn)then
                 if(i.eq.iprnin.and.j.eq.jprnin.and.nv.eq.44)then
                   write(300+iam,*)k,'g_rain',-g_rain,' E ',E_dn,'q',q(i,k),'chem_up',chem_up(i,k,nv)
                 endif
               endif
#endif
#ifdef DIAGDELL
            if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
            write(300+iam,*)'i',i,'k',k,'nv',nv,'dellactran',dellactran(i,k,nv),wetdeppart(i,k,nv)
            call flush(300+iam)
            write(500+iam,*)'i',i,'nv',nv,'top ',k+1,-zu(i,k+1)*(chem_up(i,k+1,nv)-chem_cup(i,k+1,nv)) &
              +zd(i,k+1)*(chem_down(i,k+1,nv)-chem_cup(i,k+1,nv) )*edt(i) 
            write(500+iam,*)'i',i,'nv',nv,'bot ',k,zu(i,k  )*(chem_up (i,k,nv)-chem_cup(i,k,nv  ) ) &
              -zd(i,k  )*(chem_down(i,k,nv  )-chem_cup(i,k,nv))*edt(i)
            endif
#endif
!                  if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin.and.k.eq.kprnin)then
!                 write(300+iam,*)'dellac ',i,k,nv,dellac(i,k,nv)
!                 write(300+iam,*)'chemup',chem_up(i,k:k+1,nv),nv,'cup',chem_cup(i,k:k+1,nv)
!                 write(300+iam,*)'chemdwn',chem_down(i,k:k+1,nv),'edt',edt(i)
!                 write(300+iam,*)'pw',chem_pw(i,k:k+1,nv),'pwd',chem_pwd(i,k:k+1,nv)
!                 write(300+iam,*)'zu',zu(i,k:k+1),'zd',zd(i,k:k+1),'dp',dp
!                 write(250+iam,*)'dellac ',i,k,nv,dellac(i,k,nv)
!                 call flush(300+iam)
!                 call flush(250+iam)
!                endif
!             if(isnan(dellac(i,k,nv)))then
!                write(6,*)'i',i,'k',k,'nv',nv,'ktop',ktop(i),'kte',kte,'ktf',ktf
!                write(6,*)'chemup',chem_up(i,k:k+1,nv),nv,'cup',chem_cup(i,k:k+1,nv)
!                write(6,*)'chemdwn',chem_down(i,k:k+1,nv),'edt',edt(i)
!                write(6,*)'pw',chem_pw(i,k:k+1,nv),'pwd',chem_pwd(i,k:k+1,nv)
!                call flush(6)
!             endif
            enddo
              ...
              do i=its,itf
                ...
                wetdepsol(i,:,:)=0
                 if(ierr(i).eq.0)then
                    ...
                    xmb(i)=pre(i)/max(epsilc2,denom)
                    do nv=1,numc
!                     if(lraqmschem(nv))then
                      do k=kts,ktop(i)
                          outc(i,k,nv)=dellac(i,k,nv)*xmb(i)
#ifdef DIAGDELL
                          dellactran(i,k,nv)=dellactran(i,k,nv)*xmb(i)
#endif
                          if(chemsoluable(nv))then
                             wetdeppart(i,k,nv)=wetdeppart(i,k,nv)*xmb(i)
                             wetdepsol(i,k,chempointsoluable(nv))=wetdeppart(i,k,nv)
#ifdef DIAGDELL
                             if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin.and.k.eq.1)then
                               write(6,*)'xmb',xmb(i),'pre',pre(i),'wetdeppart',wetdeppart(i,k,nv)
                               write(6,*)'dellac',dellac(i,k,nv)
                               call flush(6)
                             endif
#endif
!                             if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!                               write(300+iam,*)i,'wetdepsol',k,nv,wetdeppart(i,k,nv),'chem',chem(i,k,nv)
!                               write(300+iam,*)'chempointsoluable',chempointsoluable(nv)
!                               call flush(300+iam)
!                             endif
                          endif
                          dp=100.*(p_cup(i,k)-p_cup(i,k+1))
#ifdef DIAGDELL
                          sumdellactran(i,nv)=sumdellactran(i,nv)+dellactran(i,k,nv)*dp/g
#endif
                          sumwetdeppart(i,nv)=sumwetdeppart(i,nv)+wetdeppart(i,k,nv)*dp/g
!              if(iam.eq.iamprn.and.i.eq.iprnin.and.j.eq.jprnin)then
!                write(250+iam,*)nv,'dellac',i,k,nv,dellac(i,k,nv),'xmb',xmb(i),'denom',denom
!                call flush(250+iam)
!              endif
!                       if(isnan(outc(i,k,nv)))then
!                          write(6,*)'dellac',i,k,nv,dellac(i,k,nv),'xmb',xmb(i),'denom',denom, &
!                    'pre',pre(i),'pwav',pwav(i),edt(i),pwev(i)
!                          call flush(6)
!                       endif
                      enddo
!                      if(ktop(i)>=kts)then
!                      if(abs(sumdellactran(i,nv))>0.0.or.abs(sumwetdeppart(i,nv))>0.0)then
!                      if(abs(sumwetdeppart(i,nv))>0.0)then
!                        write(300+iam,*)'sumdellactran',i,'nv',nv,sumdellactran(i,nv),'sumwet',sumwetdeppart(i,nv)
!                        write(300+iam,*)i,'nv',nv,'sumwet',sumwetdeppart(i,nv)
!                        write(500+iam,*)'sumdellactran',i,'nv',nv,sumdellactran(i,nv),'sumwet',sumwetdeppart(i,nv)
!                        write(300+iam,*)'pre',pre(i),'other',pwav(i)+edt(i)*pwev(i)
!                        call flush(300+iam)
!                        call flush(500+iam)
!                      endif
!                      endif
...
!--- edt here is 1-precipeff!
   ...
   SUBROUTINE cup_dd_moisture_new(j,ierrc,zd,hcd,hes_cup,qcd,qes_cup,    &
...
!        qcd,qrcd,pwd
        qrcd,pwd
     real,   dimension (its:ite,kts:kte),intent(out) :: qcd
!     real*8,   dimension (its:ite,kts:kte),intent(out) :: qcd
     real,    dimension (its:ite)                                      &
        ,intent (out  )                   ::                           &
        pwev,bu
     character*50 :: ierrc(its:ite)
     real*8 denom,top,ratio
     integer j
!
!  local variables in this routine
!
...
!
      bu(i)=dz*dh
!     if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!       write(300+iam,*)'bu',bu(i),'dz',dz,'dh',dh,'hcd',k,hcd(i,k),hes_cup(i,k)
!       call flush(300+iam)
!     endif
      do ki=jmin(i)-1,1,-1
         DZ=Z_cup(i,Ki+1)-Z_cup(i,Ki)
...
!        dz=qcd(i,ki)
!        if(isnan(zd(i,ki+1)))then
!          write(6,*)'nan zd ',ki+1,zd(i,ki+1)
!          call flush(6)
!        endif
!        if(isnan(dd_massdetr(i,ki)))then
!          write(6,*)'nan dd_massdetr ',i,ki+1,dd_massdetr(i,ki)
!          call flush(6)
!        endif
!        if(isnan(dd_massentr(i,ki)))then
!          write(6,*)'nan dd_massentr ',i,ki+1,dd_massentr(i,ki)
!          call flush(6)
!        endif
!        if(isnan(qcd(i,ki+1)))then
!           write(6,*)'qcd ',i,ki+1,qcd(i,ki+1)
!           call flush(6)
!        endif
!        if(isnan(q(i,ki)))then
!          write(6,*)'q',i,ki,q(i,ki)
!          call flush(6)
!        endif
!         denom=zd(i,ki+1)-.5*dd_massdetr(i,ki)+dd_massentr(i,ki)
!         top=(qcd(i,ki+1)*zd(i,ki+1) &
!                  -.5*dd_massdetr(i,ki)*qcd(i,ki+1)+ & 
!             dd_massentr(i,ki)*q(i,ki)) 
!         if(abs(top)<1.e-20.and.abs(denom)<1.e-20)then 
!           write(400+iam,*)top,denom
!           call flush(400+iam)
!         ratio=top/denom
!         write(400+iam,*)ratio,i,ki
!         all flush(400+iam)
        
      
!         if(abs(denom)<1.e-8)then
!           write(6,*)'denom',i,ki+1,denom,'zd',zd(i,ki+1), &
!           ' detr ',dd_massdetr(i,ki),' entr ',dd_massentr(i,ki)
!           call flush(6)
!         endif
!         else
         ...
         qcd(i,ki)=(qcd(i,ki+1)*zd(i,ki+1)                          &
                  -.5*dd_massdetr(i,ki)*qcd(i,ki+1)+ &
                  dd_massentr(i,ki)*q(i,ki))   /            & 
                  max(epsilc2,zd(i,ki+1)-.5*dd_massdetr(i,ki)+dd_massentr(i,ki))
!                  (zd(i,ki+1)-.5*dd_massdetr(i,ki)+dd_massentr(i,ki)))
!         endif
!        if(isnan(qcd(i,ki)))then
!          write(6,*)'qcd nan ',i,ki,qcd(i,ki)
!          call flush(6)
!        endif
...
!
         ...
         bu(i)=bu(i)+dz*dh
!     if(iamprn>0.and.i.eq.iprnin.and.j.eq.jprnin)then
!         write(300+iam,*)'bu add ',ki,bu(i),'dz',dz,'dh',dh
!         call flush(300+iam)
!     endif
...
!      write(400+iam,*)'kbcon',i,kbcon(i),'k22',k22(i)
!      call flush(400+iam)
!      write(400+iam,*)'P-cup',P_cup(I,KBCON(I)), P_cup(I,K22(I))
!      call flush(400+iam)
      PBCDIF=-P_cup(I,KBCON(I))+P_cup(I,K22(I))
!      write(400+iam,*)'cap_max',i,cap_max(i),'iloop',iloop,'cap_inc',cap_inc(i)
!      call flush(400+iam)
...
!      endif
      do k=kts,ktop(i)
!         if(isnan(outq(i,k)))then
!            write(6,*)'nan outq',i,k
!         endif
         outq(i,k)=outq(i,k)*qmemf
         outt(i,k)=outt(i,k)*qmemf
         outu(i,k)=outu(i,k)*qmemf
...
!           call get_zu_zd_pdf_fim("UP",ierr(i),k22(i),kfinalzu,zu(i,kts:kte),kts,kte,ktf,0.,kpbli(i),csum(i))
           call get_zu_zd_pdf_fim("UP",ierr(i),k22(i),kfinalzu,zutmp,kts,kte,ktf,0.,kpbli(i),csum(i))
         zu(i,kts:kte)=zutmp
       endif
      ENDIF
     ENDDO
...
!---------------------------------------------------------------------- 
   ...
   SUBROUTINE neg_check_ct(pret,ktop,dt,q,outq,iopt,num_chem,    &
...
!-------------------------------------------------------
END MODULE raq_dep_ctrans_grell_mod
