...
!                       t_phy,moist,u_phy,v_phy,p_phy,chem,       &
    ...
    use chem_const_mod, only : epsilc,rgasuniv
    use raqmschem_species_mod, only : p_atm_shum,p_atm_cldq,idwetd
    use raqmschem_pmgrid_mod,only : iam,iamprn,iprnin,jprnin
    IMPLICIT NONE
...
!    real(CHEM_KIND_R4), dimension(ims:ime, kms:kme, jms:jme, nsol), intent(out) :: chem
    real(CHEM_KIND_R4), dimension(ims:ime, kms:kme+1, jms:jme), intent(out) :: p8w,t8w
    real(CHEM_KIND_R4), dimension(ims:ime, jms:jme), intent(out) :: pbl
    real(CHEM_KIND_R4), dimension(ims:ime, kms:kme+1, jms:jme), intent(out) :: z_at_w
