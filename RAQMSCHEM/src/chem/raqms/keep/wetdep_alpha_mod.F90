...
contains
   subroutine initwetdep(its,ite,jts,jte,kts,kte,tkin,pmid)
   use chem_types_mod
   implicit none
   integer its,ite,jts,jte,kts,kte,i,j,k
   real(CHEM_KIND_R4), dimension(its:ite,kts:kte,jts:jte), intent(in) :: tkin,pmid
   ...
   real*4,intent(in) :: t(its:ite,kts:kte),qcld(its:ite,kts:kte)
