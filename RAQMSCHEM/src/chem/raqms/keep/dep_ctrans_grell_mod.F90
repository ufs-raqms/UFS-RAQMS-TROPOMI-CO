...
!-------------------------------------------------------------
  ...
  use wetdep_fam_mod
  use raqmschem_pmgrid_mod,only : iam,iprnin,jprnin,iamprn,kprnin
!-------------------------------------------------------------
  IMPLICIT NONE
!-------------------------------------------------------------
   INTEGER,      INTENT(IN   ) ::                               &
...
!
   REAL, INTENT(IN   ) :: DT
!
   REAL, DIMENSION( ims:ime , jms:jme , kms:kme, num_chemtr ),    & ! fv3 order now
         INTENT(INOUT) ::                                       &
                                   chem
   REAL, DIMENSION( ims:ime ,           jms:jme, num_chemtr ),    &
         INTENT(INOUT) ::                                       &
                                   trfall
...
!
     ...
     ENDDO
     ENDDO
!     write(6,*)'tracer',shape(tracer)
!     write(6,*)'chem',shape(chem)
!     write(6,*)'num_chem',num_chem,num_chemtr
!     call flush(6)
     tracert=0.0
       ...
       if(ichemfullpt(nv)>0)then
         ...
         DO K=kts,ktf
           DO I=ITS,ITF
             ...
             if(iam.eq.iamprn)then
             if(i.eq.iprnin.and.j.eq.jprnin.and.k.eq.kprnin.and.nvfull.eq.p_bry)then
...
!                   endif
                   ...
                   chem(i,j,k,nvfull)=max(epsilc,chem(i,j,k,nvfull)+tracert(i,k,nv)*dt)
                   if(iamprn>0)then
...
!                     write(6,*)'chem nan grell',i,j,k,nvfull
!                     call flush(6)
!                   endif
                 ENDDO
              else
