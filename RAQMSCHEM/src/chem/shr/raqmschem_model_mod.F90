module raqmschem_model_mod

  use chem_rc_mod
  use chem_types_mod
!  use raqmschem_config_mod
  use raqmschem_config_mod, only : raqmschem_config_control_init,raqmschem_config_type, &
  raqmschem_config_read
  use chem_clock_mod
  use raqmschem_data_mod,     only : chem_data_type, chem_data_destroy
  use chem_domain_mod,   only : chem_domain_type
  use raqmschem_state_mod,    only : chem_state_type
  ...
  public :: raqmschem_model_create
  public :: chem_model_destroy
  public :: chem_model_init
  public :: raqmschem_model_get
  public :: raqmschem_model_set
  ...
  public :: raqmschem_model_domain_set
  public :: chem_model_domain_coord_set

contains

  subroutine raqmschem_model_create(deCount, rc)
...
!    write(6,*)'ajl raqmschem_model_create localdecount',localdecount
!    call flush(6)
  ...
  end subroutine raqmschem_model_create
...
!      write(6,*)'size chem-model',size(chem_model)
!      call flush(6)
      if(iam==0)then
         write(6,*)'data destroy make done',trim(cdateat)
         call flush(6)
...
!    call flush(6)
  ...
  subroutine chem_model_init(rc, comm, isolate)
    ...
    integer :: modelComm
!    write(6,*)'ajl top chem_model_init'
!    call flush(6)

    ! -- begin
    if (present(rc)) rc = CHEM_RC_SUCCESS
!    write(6,*)'ajl chem_model_init'
!    call flush(6)
...
!    call flush(6)
    ...
    do de = 0, deCount-1
      call raqmschem_model_set(de=de, modelComm=modelComm, rc=localrc)
      if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
    end do
!    write(6,*)'ajl bot chem_model_init'
!    call flush(6)
  ...
  subroutine chem_model_clock_create(rc)
    ...
    deCount = 0
!    write(6,*)'ajl chem_model_clock_create'
!    call flush(6)
    ...
    if (deCount > 0) then
      ...
      do de = 1, deCount-1
        call raqmschem_model_set(de=de, clock=model % clock, rc=localrc)
        if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
      end do
    end if
  ...
  subroutine chem_model_clock_get(julday, yy, mm, dd, h, m, s, tz, dts, &
    ...
    call raqmschem_model_get(de=0, deCount=deCount, clock=clock, rc=localrc)
    if (chem_rc_check(localrc, msg="Failure to retrieve model on local DE", &
      file=__FILE__, line=__LINE__, rc=rc)) return
  ...
  subroutine chem_model_config_init(rc)
    ...
    ! --       config points to DE 0 on other DEs on the same PET
    deCount = 0
!    write(6,*)'ajl chem_model_config_init'
!    call flush(6)
    call chem_model_local_get(de=0, deCount=deCount, model=model, rc=localrc)
    if (chem_rc_check(localrc, msg="Failure to retrieve model on local DE", &
      file=__FILE__, line=__LINE__, rc=rc)) return
    ...
    if (deCount > 0) then
      ...
      call raqmschem_config_read(model % config, rc=localrc)
      if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
      ! -- setup internal parameters based on input options
      call raqmschem_config_control_init(model % config, rc=localrc)
      if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
      if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
      ! -- populate pointers on other local DEs
!      write(6,*)'ajl decount',decount,'call set'
!      call flush(6)
      do de = 1, deCount-1
        call raqmschem_model_set(de=de, config=model % config, rc=localrc)
        if (chem_rc_check(localrc, file=__FILE__, line=__LINE__, rc=rc)) return
      end do
    end if
!    write(6,*)'ajl bot chem_model_config_init'
!    call flush(6)

  end subroutine chem_model_config_init


  subroutine raqmschem_model_domain_set(minIndexPDe, maxIndexPDe, minIndexPTile, maxIndexPTile, &
    ...
    !-- begin
    if (present(rc)) rc = CHEM_RC_SUCCESS
!    write(6,*)'ajl raqmschem_model_domain_set'
!    call flush(6)
  ...
  end subroutine raqmschem_model_domain_set
  ...
  subroutine raqmschem_model_set(de, numIntLayers, numModLayers, numSoilLayers, numTracers, modelComm, tileComm, &
    ...
    type(raqmschem_config_type), optional, pointer :: config
    integer, optional, intent(out) :: rc
    ...
    ! -- begin
    if (present(rc)) rc = CHEM_RC_SUCCESS
!    write(6,*)'ajl raqmchem_model_set'
!    call flush(6)
    call chem_model_local_get(de=de, deCount=deCount, model=model, rc=localrc)
    if (chem_rc_check(localrc, msg="Failure to retrieve model on local DE", &
      file=__FILE__, line=__LINE__, rc=rc)) return
  ...
  end subroutine raqmschem_model_set


  subroutine raqmschem_model_get(de, deCount, stateIn, stateOut, clock, config, &
    ...
    localDeCount = 0
!    !write(6,*)'call chem_model_local_get'
!    call flush(6)    
    call chem_model_local_get(de=de, deCount=localDeCount, model=model, rc=localrc)
!     write(6,*)'ajl raqmchem_model_get chem_model_local_get',localrc
!     call flush(6)
    if (chem_rc_check(localrc, msg="Failure to retrieve model on local DE", &
      file=__FILE__, line=__LINE__, rc=rc)) return
    ...
    if (localDeCount > 0) then
!      write(6,*)'localdecount',localdecount
!      call flush(6)
  ...
  end subroutine raqmschem_model_get
...
!    write(6,*)'ajl local get de',localDe
!    call flush(6)
!    if(present(decount))then
!      write(6,*)'ajl local get decount',decount
      !call flush(6)
!    endif
    ...
    if (allocated(chem_model)) then
!      write(6,*)'ajl chem_model_local_get sizechemmodel',size(chem_model)
!      call flush(6)
...
!    write(6,*)'ajl bot chemmodelelocalget'
!    call flush(6)

  end subroutine chem_model_local_get


end module raqmschem_model_mod
