...
!  write(6,*)'rarea',maxval(rarea),minval(rarea)
  ...
  if(mype.eq.1)then
    ...
    write(6,*)'u3dwithhalo',maxval(u3dwithhalo),minval(u3dwithhalo)
    write(6,*)'v3dwithhalo',maxval(u3dwithhalo),minval(u3dwithhalo)
  endif
#endif
!  vort=1000.
! calculate vorticity
...
!        ud(i,jhs)=2.*ud(i,jhs+1)-ud(i,jhs+2)
        ...
        if(k.eq.63.and.i.eq.40.and.mype<=1)then
          ...
          write(70+mype,*)'u3withhalo',jhs,u3dwithhalo(i,jhs,k),jhs+1,u3dwithhalo(i,jhs+1,k)
...
!    do j=jds,jde,jde-jds
    if(jhs.eq.jds)then
      do i=ids,ide
        first=vort(i,jds+2,k)-vort(i,jds+1,k)
        second=.5*(vort(i,jds+3,k)-2.*vort(i,jds+2,k)+vort(i,jds+1,k))
        if(i.eq.40.and.mype<=1.and.k.eq.63)then
          write(60+mype,*)'first',first,'second',second,'vort2',vort(i,jds+2,k)
          call flush(60+mype)
        endif
        vort(i,jds,k)=vort(i,jds+1,k)-first+second
        if(i.eq.40.and.mype<=1.and.k.eq.63)then
          write(80+mype,*)'vort one',vort(i,jds,k),'vort2',vort(i,jds+1,k),vort(i,jds+2,k)
          write(80+mype,*)'first ',-first,'second',second
          call flush(80+mype)
        endif
      end do
    endif
...
!       upper left corner
    ...
    if(ihe.eq.ide)then
      do j=jhs+1,jhe-1 ! skip corner for now
        ...
        if(j.eq.10.and.k.eq.63.and.mype<=1)then
          write(80+mype,*)ide-3,vort(ide-3,j,k),vort(ide-2,j,k),vort(ide-1,j,k)
          write(80+mype,*)'first',first
          write(80+mype,*)'second',second
          call flush(80+mype)
        endif
      end do
      if(jhs.eq.jds)then
...
!   handle corners
    if(k.eq.63.and.mype<=1)then
      if(jhs.eq.jds)then
        i=40
        do j=jds+6,jds,-1
          write(80+mype,*)j,'vort',vort(i,j,k),vort(i,j+1,k)-vort(i,j,k),vort(i,j+2,k)-2.*vort(i,j+1,k)+vort(i,j,k)
          write(80+mype,*)j,'first center ',.5*(vort(i,j+2,k)-vort(i,j,k)), &
          .5* (vort(i,j+3,k)-2.*vort(i,j+2,k)+vort(i,j+1,k)+vort(i,j+2,k)-2.*vort(i,j+1,k)+vort(i,j,k))
          call flush(80+mype)
        end do
      endif
    endif
