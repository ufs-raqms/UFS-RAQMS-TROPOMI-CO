...
! USE module_mixactivate_wrappers, only: mosaic_mixactivate, sorgam_mixactivate
  ...
  IMPLICIT NONE
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme, num_chem ),         &
         INTENT(INOUT ) ::                                 chem
!   real, dimension(ims:ime,kms:kme,jms:kme) :: chemtend ! ajl
   real  :: chemtend ! ajl
...
!
  ...
  character*10 cdiagmix
  logical :: diagmix=.false.
...
! .. Intrinsic Functions ..
  ...
  cdiagmix=' '
  call getenv("DIAGMIX",cdiagmix)
  if(cdiagmix=='YES')then
    diagmix=.true.
  endif
...
!      CALL wrf_debug(15,'DOING DRY DEP VELOCITIES WITH WESELY METHOD')
      if(diagmix)then
        write(6,*)'chem_opt',chem_opt,'simple',GOCART_SIMPLE
      endif
...
!
    ...
    if(diagmix)then
      write(6,*)'ddep',maxval(ddep),minval(ddep),'num_chem',num_chem
      write(6,*)'ddvel',maxval(ddvel),minval(ddvel)
      call flush(6)
    endif
...
!           endif
         ...
         if(diagmix)then
           do k=kts,kte
!             chemtend(i,k,j)=pblst(k)-chem(i,k,j,nv)
             chemtend=max(epsilc,pblst(k))-max(epsilc,chem(i,k,j,nv))
             if(i==its.and.j==jts)then
!               write(6,*)'chemtend',nv,k,chemtend(i,k,j),'ddvel',ddvel(i,j,nv)
               if(k>1.and.abs(chemtend)>1.e-5*pblst(k))then
               write(200+mype,*)'chemtend',nv,k,chemtend,'pblst',pblst(k),chem(i,k,j,nv)
               call flush(200+mype)
               endif
             endif
           end do
