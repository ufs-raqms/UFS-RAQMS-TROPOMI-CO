...
subroutine update_atmos_chemistry(state, rc)
  ...
  real(ESMF_KIND_R8), dimension(:,:,:,:), pointer :: q_raqms ! ajl
  real(ESMF_KIND_R8), dimension(:,:),pointer :: cmdrag ! ajl
  real(ESMF_KIND_R8), dimension(:,:),pointer :: cktopkbotcnv ! ajl
  real(ESMF_KIND_R8), dimension(:,:,:),pointer :: cldtau ! ajl
  real(ESMF_KIND_R8), dimension(:,:,:),pointer :: vort ! ajl
...
! ajl begin
  ...
  logical raqmso3mr
  save raqmso3mr
  data raqmso3mr/.false./
  logical first
  save first
  data first/.true./
  real(ESMF_KIND_R8),parameter :: mwo3=47.98,mwair=28.97
  integer :: io3mr,io3vmr,iox,mype ! ajl
  integer :: ico ! ajl
  mype=mpp_pe()
  cenv=' '
!   write(6,*)'ajl top update'
!   call flush(6)
!  write(200+mype,*)'ajl top update chemistry'
!  call flush(200+mype)
  call getenv('RAQMSO3MR',cenv)
  io3mr = get_tracer_index(MODEL_ATMOS, 'o3mr')
  iox = get_tracer_index(MODEL_ATMOS, 'ox' )
  ico = get_tracer_index(MODEL_ATMOS, 'co' )
!  iso2 = get_tracer_index(MODEL_ATMOS, 'so2' )
!  if (mpp_pe() == mpp_root_pe())then
!  write(6,*)'io3mr',io3mr,' iox ',iox
!  call flush(6)
!  endif
  if(cenv.eq.'YES'.or.cenv.eq.'yes')then
    if (mpp_pe() == mpp_root_pe())then
      if(first)then
      write(6,*)'find ajl use raqmso3mr'
      call flush(6)
      first=.false.
      endif
    endif
...
! end ajl begin
  ...
  select case (trim(state))
    case ('import')
      ...
      call cplFieldGet(state,'raqms_inst_tracer_mass_frac', farrayPtr4d=q_raqms, rc=localrc) ! aJL 
      if (ESMF_LogFoundError(rcToCheck=localrc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__, rcToReturn=rc)) return
...
!          if(it<20.and.ib==140.and.jb==101.and.k<=64)then
!            if(it>4.and.q(i,j,k,it)<0.0)then
!              write(6,*)'qfromchem neg','k',k,q(i,j,k,it),'it',it
!            endif
!            write(6,*)'qfromchem',it,'i',i,j,'k',k,q(i,j,k,it),'ib',ib,'jb',jb,'nb',nb,'ix',ix
!          endif
            enddo
          enddo
        enddo
...
!     ajl add
      if(iox /= NO_TRACER)then
        do it=nte+1,nt
!$OMP parallel do default (none) &
!$OMP             shared  (it, nk, nj, ni, Atm_block, IPD_Data, q_raqms,ico)  &
!$OMP             private (k, j, jb, i, ib, nb, ix)
        do k = 1, nk
          do j = 1, nj
            jb = j + Atm_block%jsc - 1
            do i = 1, ni
              ...
              nb = Atm_block%blkno(ib,jb)
              ix = Atm_block%ixp(ib,jb)
              IPD_Data(nb)%Stateout%gq0(ix,k,it) = q_raqms(i,j,k,it)
!              if(it.eq.ico.and.ib.eq.80.and.jb.eq.123)then
!                 write(6,*)'nb',nb,'i',i,'j',j,'k',k,q_raqms(i,j,k,it)
!              endif
            enddo
          enddo
        enddo
        ...
        do k = 1, nk
          do j = 1, nj
            ...
            do i = 1, ni
              ...
              nb = Atm_block%blkno(ib,jb)
              ix = Atm_block%ixp(ib,jb)
              IPD_Data(nb)%Stateout%gq0(ix,k,io3mr) = q_raqms(i,j,k,io3mr)
            enddo
          enddo
        enddo
      endif
...
!$OMP             private (j, jb, i, ib, nb, ix)
    ...
    case ('export')
      ...
      call cplFieldGet(state,'inst_cldtau', farrayPtr3d=cldtau, rc=localrc) !  ajl
      call cplFieldGet(state,'inst_vort', farrayPtr3d=vort, rc=localrc) ! ajl
...
! ajl begin
      call cplFieldGet(state,'cm_drag', farrayPtr2d=cmdrag, rc=localrc) ! ajl
      if (ESMF_LogFoundError(rcToCheck=localrc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__, rcToReturn=rc)) return
      call cplFieldGet(state,'ktop_kbot_cnv', farrayPtr2d=cktopkbotcnv, rc=localrc) ! ajl
      if (ESMF_LogFoundError(rcToCheck=localrc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__, rcToReturn=rc)) return
! ajl end
      ...
      cldtau=0.0 ! ajl
      vort=0.0 ! ajl
!      write(6,*)'cldtau',shape(cldtau),ni,nj,nk
!      call flush(6)

!$OMP parallel do default (none) &
!$OMP             shared  (nk, nj, ni, Atm_block, IPD_Data, prsi, phii, prsl, phil, temp, ua, va, vvl, dkt, dqdt,cldtau,vort)  &
!$OMP             private (k, j, jb, i, ib, nb, ix)
      do k = 1, nk
        do j = 1, nj
          jb = j + Atm_block%jsc - 1
...
!      write(6,*)nb,ix,k,'allocated cldtau_frac',allocated(IPD_Data(nb)%Coupling%cldtau_frac)
!      call flush(6)

            cldtau(i,j,k)=IPD_Data(nb)%Coupling%cldtau_frac(ix,k) ! ajl
            vort(i,j,k)=IPD_Data(nb)%Coupling%vort(ix,k) ! ajl
...
!$omp parallel do default(shared) private(i,j,jb,ib,nb,ix)
      ...
      do it = 1, nt
!$OMP parallel do default (none) &
!$OMP             shared  (it, nk, nj, ni, Atm_block, IPD_Data, q,mype)  &
!$OMP             private (k, j, jb, i, ib, nb, ix)
        do k = 1, nk
          do j = 1, nj
            jb = j + Atm_block%jsc - 1
...
!          if(it<=24.and.ib==140.and.jb==101.and.k==1.and.mype==34)then
!          if(it<=24.and.ib==140.and.jb==101.and.k<=64)then
!            if(it>4.and.q(i,j,k,it)<0.0)then
!              write(6,*)'qtochemneg k',k,q(i,j,k,it),'it',it
!            endif
!           write(6,*)'qto chem',it,'k',k,q(i,j,k,it),'i',i,j
!         endif
            enddo
          enddo
        enddo
!        write(6,*)'qchem',it,maxval(q(:,:,:,it)),minval(q(:,:,:,it))
      enddo

!$OMP parallel do default (none) &
!$OMP             shared  (nj, ni, Atm_block, IPD_Data, &
!$OMP                      hpbl, area, stype, rainc, rain, uustar, sfcdsw, &
!$OMP                      cmdrag,cktopkbotcnv, & ! ajl
...
!$OMP             private (j, jb, i, ib, nb, ix)
      do j = 1, nj
        ...
        do i = 1, ni
          ...
          cmdrag(i,j)  = IPD_Data(nb)%sfcprop%cmdrag(ix) ! ajl
          cktopkbotcnv(i,j)=IPD_Data(nb)%Coupling%cktopkbotcnv(ix) ! ajl
