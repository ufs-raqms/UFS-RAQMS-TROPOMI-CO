...
!-----------------------------------
   ...
   if (max(sphum,liq_wat,ice_wat,rainwat,snowwat,graupel) > Atm(mytile)%flagstruct%nwat) then
     write(6,*)'nwat', Atm(mytile)%flagstruct%nwat
     write(6,*)'graupel',graupel,sphum,liq_wat,ice_wat,rainwat,snowwat
     call flush(6)
...
!! and compute a consistent prognostic state.
 subroutine atmosphere_state_update (Time, IPD_Data, IAU_Data, Atm_block, flip_vc)
   ...
   real(kind=kind_phys),allocatable :: vort(:,:,:) ! ajl
...
!--- physics tendencies
  ...
  !---- diagnostics for FV dynamics -----
   if (Atm(mytile)%flagstruct%print_freq /= -99) then
     ...
     allocate (vort(isc:iec,jsc:jec,npz)) ! ajl
!    ajl add vort 
     call fv_diag(Atm(mytile:mytile), zvir, fv_time, Atm(mytile)%flagstruct%print_freq,vort=vort)
! start ajl
     do nb = 1,Atm_block%nblks
       blen = Atm_block%blksz(nb)
       do k = 1, npz
         ...
         do ix = 1, blen 
           i = Atm_block%index(nb)%ii(ix)
           j = Atm_block%index(nb)%jj(ix)
           IPD_Data(nb)%Coupling%vort(ix,k) = vort(i,j,k1)
          enddo
       enddo
     enddo
! end ajl
