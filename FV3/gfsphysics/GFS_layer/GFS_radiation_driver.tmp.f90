...
# 1 "GFS_layer/GFS_radiation_driver.F90" 2
...
# 1046 "GFS_layer/GFS_radiation_driver.F90"
...
!       if (.not. Model%uni_cld) then
        if (Model%lgocart .or. Model%ldiag3d) then
          ...
          if (Model%lsswr)then ! ajl
            if(nday > 0) then ! new ajl
              do k = 1, LM
                k1 = k + kd 
                Coupling%cldtau_frac(:,k) = cldtausw(:,k) ! ajl
              enddo
            else 
!             cldtausw not calculated
              do k = 1, LM
                k1 = k + kd 
                Coupling%cldtau_frac(:,k) = 0.0 ! ajl
              enddo
            endif
          endif
        endif
