...
!     subroutine samfshalcnv(im,ix,km,delt,ntk,ntr,delp,
     ...
     &     clam,c0s,c1,pgcon,asolfac,wet_shallow)!lzhang
!
...
!  for aerosol transport
      ...
      real(kind=kind_phys) wet_shallow(im,ntc) !lzhang
...
!
      ...
      real(kind=kind_phys)  :: changeup ! ajl
      logical do_chem,first,sastracerpos
      save do_chem,ndochem,idochem,sastracerpos
      data do_chem/.false./,first/.true./,sastracerpos/.false./
      integer ndochem,idochem,nnegative(ntr)
      real(kind=kind_phys),allocatable :: qchem(:,:,:)
      character *10 cenv
      integer me,nb,meat,nn
      common/myme2/me
      common/mynb/nb
      meat=34
      meat=18
...
!>  ## Determine whether to perform aerosol transport
      ...
      if(first)then
        cenv=' '
        call getenv('SASSHALTRACERPOS',cenv)
        if(cenv.eq.'YES')then
          sastracerpos=.true.
!          if(me.eq.0)write(6,*)'SASSHALTRACERPOS'
        endif
      endif
      if (do_aerosols)then
        if(ntr> itc+ntc-3)then
          if(first)then
            cenv=' '
            call getenv('SAMFCHEM',cenv)
            if(cenv=='YES')then
              do_chem=.true.
            endif
!           do_chem=.true.
!            write(200+me,*)'ndochem',ndochem,'idochem',idochem,
!     *      'ntc',ntc,'itc',itc,'ntr',ntr
            first=.false.
          endif
        endif
      endif
      ndochem=ntr-(itc+ntc-1)
      idochem=itc+ntc
      if(do_chem)then
        allocate (qchem(im,km ,ndochem))
      endif
!
!************************************************************************
...
!
      do n = 3, ntr+2
        ...
        do k = 1, km
          do i = 1, im
            if (cnvflg(i) .and. k <= kmax(i)) then
!              if(n<24.and.nb==8.and.i==12.and.me==meat)then
!                write(6,*)'n shalcnv chem in',n,'k',k,qtr(i,k,n),'kk',kk
!                call flush(6)
              !endif
...
!    &  edto, xlamd, xmb, c0t, eta, etad, zi, xlamue, xlamud, delp,
     ...
     &  qtr, qaero)
      ...
      if(do_chem)
!     &  call samfshalcnv_chem(im, ix, km, itc, ntc, ntr, delt,
     &  call samfshalcnv_chem(im, ix, km, idochem, ndochem, ntr, delt,
...
!             v1(i,k) = v1(i,k) + dellav(i,k) * xmb(i) * dt2 * tem
        ...
        enddo
      enddo
      if(sastracerpos)then

!      do n = 1, ntr
!        kk = n+2
        do nn = 1, ndochem ! new limits
          kk = nn + idochem - 1
          n=kk-2
        do k = 1, km
          do i = 1, im
            if (cnvflg(i) .and. k <= kmax(i)) then
              if(k <= ktcon(i)) then
                  if(-dellae(i,k,n)*xmb(i)*dt2>ctr(i,k,n))then
!                   make 10 percent a limit
                    changeup=-(dellae(i,k,n)*xmb(i)*dt2
     *                +ctr(i,k,n))+.1*ctr(i,k,n)
                    dellae(i,k,n)=dellae(i,k,n)+changeup/(xmb(i)*dt2)
                    dellae(i,k+1,n)=dellae(i,k+1,n)-changeup*del(i,k)/
     *                (xmb(i)*dt2)/del(i,k+1)
!                  write(6,*)'shall n',n,'k',k
!                  call flush(6)
                  endif
          ...
          do k = 1, km
            do i = 1, im
              if(cnvflg(i) .and. rn(i) > 0.) then
                ...
                wet_shallow(i,n)=wet_shallow(i,n)
     &  + ((qtr(i,k,kk)-qaero(i,k,n))*delp(i,k)/(g*delt))
                qtr(i,k,kk) = qaero(i,k,n)
!              if(kk<24.and.nb==8.and.i==12.and.me==meat)then
!         write(6,*)'chem shallcnv out',n,'k',k,qaero(i,k,n),'kk',kk
!              endif
                endif
              endif
!              if(qtr(i,k,kk) <0.0)then
!                if(kk<24.and.nb==8.and.i==12.and.me==meat)then
                   !write(6,*)'shallcnv neg',k,qtr(i,k,kk),'kk',kk
!                endif
              !endif
            enddo
          enddo
        ...
        do n = 1, ndochem
          ...
          do k = 1, km
            do i = 1, im
              if(cnvflg(i) .and. rn(i) > 0.) then
                if (k <= kmax(i))then
                if(qchem(i,k,n)<=0.0)then
!               write(200+me,*)'qchem shallow neg',i,'k',k,kk,n,
!     *qchem(i,k,n),'qtr',qtr(i,k,kk),'kmax',kmax(i)
                  !call flush(200+me)
                endif
                endif
                if (k <= kmax(i)) qtr(i,k,kk) = qchem(i,k,n)
!              if(k<=kmax(i))then
!              if(kk<24.and.nb==8.and.i==12.and.me==meat)then
!       write(6,*)'dochem shallcnv out',n,'k',k,qaero(i,k,n),'kk',kk
!              endif
!              endif
              endif
            enddo
