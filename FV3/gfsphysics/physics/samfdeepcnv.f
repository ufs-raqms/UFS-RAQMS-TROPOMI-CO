...
c    &            .743,.813,.886,.947,1.138,1.377,1.896/
      ...
      save do_chem,first,sastracerpos
...
!      if(me==meat)write(6,*)'nb',nb,'ntr',ntr,'ntc',ntc,'itc',itc,'ix',ix
      if (first)then
        cenv=' '
       call getenv('SASTRACERPOS',cenv)
       if(cenv.eq.'YES')then
          sastracerpos=.true.
!          if(me.eq.0)write(6,*)'SASTRACERPOS'
        endif
        if(do_aerosols)then
          if(ntr> itc+ntc-3)then
            ...
            if(cenv=='YES')then
              do_chem=.true.
!             if(first)then
!               write(6,*)'SAMFCHEM DEEP'
!             endif
            endif
!           so2 is itc with ntc=20 more then comes out chemicals
          endif
        endif
        first=.false.
      endif
      ...
      idochem=itc+ntc
!        write(300+me,*)'ndochem',ndochem,'itc',itc,'ntc',ntc,'ntr',ntr
!        write(300+me,*)'idochems',idochem
      if(do_chem)then
...
!                write(6,*)'n chem in',n,'k',k,qtr(i,k,n),'kk',kk
!                call flush(6)
!              endif
...
c
     ...
     &  qtr, qaero)
      ...
      if (do_chem)
     &  call samfdeepcnv_chem(im, ix, km, idochem, ndochem, ntr, delt,
...
!                write(6,*)'sastracerpos n',n,'kk',kk,'k',k
!                write(6,*)'idochem',idochem,ndochem,'itc',itc,ntc
!              endif
...
!              if(kk<24.and.nb==8.and.i==12.and.me==meat)then
!                  write(6,*)'chem out',n,'k',k,qaero(i,k,n),'kk',kk
!              endif
                endif
              endif
!              if(qtr(i,k,kk)<0.0)then
!              if(kk<24.and.nb==8.and.i==12.and.me==meat)then
!                 write(6,*)'qtr deepneg',k,qtr(i,k,kk),'kk',kk
!                 call flush(6)
!              endif
!              endif
            enddo
              ...
              if(cnvflg(i) .and. rn(i) > 0.) then 
                if (k <= kmax(i))then
                  if(qchem(i,k,n)<=0.0)then
                    write(6,*)'qchem neg',i,k,n,'kk',kk,qchem(i,k,n),
     *    qtr(i,k,kk)
                  endif
                endif
                if (k <= kmax(i)) qtr(i,k,kk) = qchem(i,k,n)
!      if(kk<24.and.nb==8.and.i==12.and.me==meat.and.kk==9)then
!                  write(6,*)'sulf out chem',k,qtr(i,k,kk)
!                endif
              endif
            enddo
          enddo
