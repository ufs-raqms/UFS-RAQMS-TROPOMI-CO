...
! 
      IF(PRINT_ESMF) THEN
        CALL ESMF_Initialize(VM             =VM                         & !<-- The ESMF Virtual Machine
                            ...
                            ,rc             =RC)
        if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=143, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
      ELSE
        CALL ESMF_Initialize(VM             =VM                         & !<-- The ESMF Virtual Machine
                            ,defaultCalKind =ESMF_CALKIND_GREGORIAN     & !<-- Set up the default calendar.
                            ,logkindflag    =ESMF_LOGKIND_NONE          & !<-- Define no log error output files;
                            ,rc             =RC)
        if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=149, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
      ENDIF
!
!-----------------------------------------------------------------------
...
!
      CALL ESMF_VMGet(vm      =VM                                       &  !<-- The ESMF Virtual Machine
                     ...
                     ,rc      =RC)
      if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=171, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      if (mype==0) call w3tagb('NEMS '//"(mbruckner) Tue Jan 3 22:57:25 UTC 2023 r40c8d807b9f2 https://gitlab.ssec.wisc.edu/lenzen/fv3gfs-v9.1.git",                       &
                               2023, 003, 0000, 'NEMS')
!
!-----------------------------------------------------------------------
...
!                        ,logmsgAbort=(/ ESMF_LOGMSG_ERROR /)            &
                        ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=210, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
      ENDIF
...
!
      CALL ESMF_ConfigLoadFile(config  =CF_MAIN                         &  !<-- The Configure object
                              ...
                              ,rc      =RC)
      if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=230, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
!-----------------------------------------------------------------------
...
!
      NEMS_GRID_COMP=ESMF_GridCompCreate(name        ='NEMS Grid Comp'  &  !<-- NEMS component name
                                        ...
                                        ,rc          =RC)
      if (ESMF_LogFoundError(RC, msg="Aborting NEMS", line=247, file="MAIN_NEMS.F90")) call ESMF_Finalize(endflag=ESMF_END_ABORT)
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
!-----------------------------------------------------------------------
