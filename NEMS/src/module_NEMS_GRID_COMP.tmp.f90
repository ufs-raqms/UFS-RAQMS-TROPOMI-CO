...
!      write(6,*)'total_member',total_member
!      call flush(6)
!
!-----------------------------------------------------------------------
...
! 
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = TRIM(PELAB)               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=413, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
...
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = 'HH_FINAL:'               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=475, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
...
!
      DO I = 1, TOTAL_MEMBER
        EARTH_GRID_COMP(i) = ESMF_GridCompCreate (                      &
                            ...
                            ,rc           = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=500, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
...
!
      IF(ENS_SPS) THEN
        ENS_CPL_COMP=ESMF_CplCompCreate(name = "ENS Cpl component"      &  
                                       ,rc   = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=511, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1, TOTAL_MEMBER
        CALL ESMF_GridCompSetServices(EARTH_GRID_COMP(I)                &  !<-- The EARTH gridded components
                                     ...
                                     ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=528, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
!***  Register the Initialize, Run, and Finalize routines of
...
!
        CALL ESMF_CplCompSetServices(ENS_CPL_COMP                       &
                                    ...
                                    ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=546, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_IMP_STATE(I) = ESMF_StateCreate(                          &
                                        ...
                                        ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=564, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
!
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_EXP_STATE(I) = ESMF_StateCreate(                                 &
                                        ...
                                        ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=579, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
...
!
        ENS_CPL_EXP_STATE=ESMF_StateCreate(name = "ENS_CPL_Export"           &
                                          ,stateintent = ESMF_STATEINTENT_EXPORT  &
                                          ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=608, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
! 
        DO I = 1, TOTAL_MEMBER
          IF(MEMBER_ID == I) THEN
            ...
            if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=623, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
            CALL ESMF_StateAddReplace(ENS_CPL_EXP_STATE,(/EARTH_IMP_STATE(I)/), rc = RC)
            if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=625, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
          END IF
        END DO
!
      END IF
...
!-----------------------------------------------------------------------
      ...
      if (fhrot > 0) then
        ...
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=647, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        CURRTIME = STARTTIME + restartOffset
        call ESMF_ClockSet(CLOCK_NEMS, currTime=CURRTIME, rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=650, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      endif
!
!-----------------------------------------------------------------------
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompInitialize(gridcomp    = EARTH_GRID_COMP(I)  &
                                      ...
                                      ,userRc      = RC_USER             &
                                      ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=673, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=674, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        END IF
!
      END DO
!
...
!
        CALL ESMF_CplCompInitialize(cplcomp    =ENS_CPL_COMP            &
                                   ...
                                   ,userRc     =RC_USER                 &
                                   ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=697, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=698, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(I)        &
                               ...
                               ,userRc      = RC_USER                   &
                               ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=774, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=775, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
        END IF
!
      END DO
!
...
!
          CALL ESMF_CplCompRun(cplcomp    =ENS_CPL_COMP                 &
                              ...
                              ,userRc     =RC_USER                      &
                              ,rc         =RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=802, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=803, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_VMBarrier(vm = VM_GLOBAL, rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=806, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
...
!
          CALL ESMF_ClockGet(clock       = CLOCK_NEMS                   &
                            ...
                            ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=820, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
...
!
          CALL ESMF_TimeIntervalGet(timeInterval = RUNDURATION          &
                                   ,h            = HH                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=837, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          HH = HH + HH_INCREASE
!
          CALL ESMF_TimeIntervalSet(timeInterval = RUNDURATION          &
                                   ,h            = hh                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=844, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_ClockSet(clock       = CLOCK_NEMS                   &
                            ,runDuration = RUNDURATION                  &
                            ,rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=849, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!***  Execute the Run step of each element in the EARTH component
!***  array.
...
!
            IF(MEMBER_ID == J) THEN
                CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(J)  &
                                     ...
                                     ,userRc      = RC_USER             &
                                     ,rc          = RC)
                if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=871, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
                if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=872, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
            END IF
!
          END DO
!
...
!
         CALL ESMF_ClockGet(clock       = CLOCK_NEMS                    &
                           ,runDuration = RUNDURATION                   &
                           ,rc          = RC)
         if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=887, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!
      END IF sps
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompFinalize(gridcomp    = EARTH_GRID_COMP(I)   &
                                    ...
                                    ,userRc      = RC_USER              &
                                    ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=954, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=955, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        END IF
!
      END DO
!
...
!
        CALL ESMF_CplCompFinalize(cplcomp    =ENS_CPL_COMP              &
                                 ...
                                 ,userRc     =RC_USER                   &
                                 ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=976, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=977, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
!
      END IF
!
!-----------------------------------------------------------------------
