...
$GSI_DIR/run/run_gsi_TROPOMI.CO.ak.thin25km.ksh >& $GSI_SCRATCH/outgsi.TROPOMI.CO.thin25km.$CDATEF
# add OMPS now
export firstscript=.true.
export YYYY=`echo $CDATE | cut -c1-4`
export base_data_disk_OMPS_OZONE=/ships19/aqda/lenzen/OMPS-NPP/${YYYY}
$GSI_DIR/run/run_gsi_FV3GFS.OMPS.OZONE.ksh  >& $GSI_SCRATCH/outgsi.OMPS.OZONE.$CDATEF
# need to do outside of script above
# add VIIRS AOD now
echo YYYY $YYYY
export base_data_disk_viirs_aod=/ships19/aqda/lenzen/VIIRS.NC/${YYYY}
$GSI_DIR/run/rungsi.FV3GFS.GSI.VIIRS.AOD.SEAS12.sh >& $GSI_SCRATCH/outgsi.VIIRS.AOD.SSEAS1.$CDATEF
